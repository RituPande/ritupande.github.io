...
This expression is termed as $$ELBO(X, Q, \Theta )$$  or the **E**vidence **L**ower **Bo**und and based on  the above derivation, it is clear that $$L(\Theta| X )$$ is always greater than this value. However, Jenson's inequality also says that ELBO will be exactly equal to $$L(\Theta| X )$$   
