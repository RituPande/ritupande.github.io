...
*Likelihood function* $$\displaystyle L(\Theta \mid X ) = \sum_{i=1}^N log (P(x^{(i)}; \Theta))$$    
...
By definition of Expectation: $$\displaystyle = \sum_{i=1}^N log ( E_{z \sim Q(Z^{(i)})}\frac{P(x^{(i)},z, \Theta ) }{ Q(z) })$$   

Since $$log$$ is a concave function: using Jenson's inequaltity: $$\displaystyle \ge \sum_{i=1}^N E_{z \sim Q(Z^{(i)})}log( \frac{P(x^{(i)},z, \Theta ) }{ Q(z) })$$     
